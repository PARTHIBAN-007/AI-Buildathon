...
class VoiceService:
    ...
    async def _cancel_all_pending_jobs_for_checkout(
        self, checkout_id: str | None = None, phone: str | None = None
    ) -> int:
    ...
    async def trigger_call(
        self, phone: str, language: str | None = None, metadata: Dict | None = None
    ) -> Dict[str, Any]:
        logger.info(f"VoiceService.trigger_call phone={phone}")
        m = metadata or {}
        return await self.client.trigger_call(
            phone=phone,
            checkout_id=m.get("checkout_id"),
            call_summary=m.get("call_summary"),
            opening_line=m.get("opening_line"),
            user_name=m.get("user_name"),
            connection_id=m.get("connection_id") or getattr(settings, "EXOTEL_ACCOUNT_SID", None),
            agent_phone_number=m.get("agent_phone_number") or getattr(settings, "EXOTEL_PHONE_NUMBER", None),
            webhook_url=m.get("webhook_url"),
            lead_id=m.get("lead_id"),
        )

    async def trigger_immediate_call(
        self, phone: str, checkout_id: str | None = None, metadata: Dict | None = None
    ) -> Dict[str, Any]:
        """Trigger an immediate voice call and record it in ScheduledJob."""
        ...
        payload = {"checkout_id": checkout_id, "phone": phone, **(metadata or {})}
    ...
    async def schedule_call(
        ...
        job = await create_scheduled_job(
            ...
            job_type="voice",
            status="SCHEDULED",
            scheduled_at = datetime.now(timezone.utc) + timedelta(seconds=eta_seconds),
        )
    ...
    async def reschedule_call(self, job_id: str, new_eta_seconds: int) -> dict:
        ...
        target_phone = job.phone or "placeholder"
        payload = {"checkout_id": job.checkout_id, "phone": target_phone}
        task = celery_app.send_task(
            "tasks.trigger_sarvam_voice_call",
            args=[target_phone, payload],
            countdown=new_eta_seconds,
        )
